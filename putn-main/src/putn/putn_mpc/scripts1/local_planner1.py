...
class Local_Planner():
    def __init__(self):
        ...
        self.__sub_obs = rospy.Subscriber('obs', Float32MultiArray, self.__obs_cb, queue_size=1)
        self.__sub_goal_state = rospy.Subscriber('surf_predict_pub', Float32MultiArray, self._global_path_callback2, queue_size=1)
        self.__pub_local_path = rospy.Publisher('local_path', Path, queue_size=1)
        self.__pub_local_plan = rospy.Publisher('local_plan', Float32MultiArray, queue_size=1)
        self.control_cmd = Twist()
        self.listener = tf.TransformListener()
        self.times = 0
        self.obstacle_markerarray = MarkerArray()
        self.ob_pub = rospy.Publisher('ob_draw', MarkerArray, queue_size=1)
