...
std::string camera_init_id, aft_mapped_id;
...
{ 
  ...
  listener_ptr->waitForTransform("world", aft_mapped_id, ros::Time(0), ros::Duration(2.0));
  ...
  for (const auto& pt : cloud_filt->points)
  {
    ...
    origin_point.header.frame_id = aft_mapped_id;
