...
{   
    ...
    {
        if(pt.x -dila< lowerbound_(0)) lowerbound_(0)=pt.x-dila;
        if(pt.y -dila< lowerbound_(1)) lowerbound_(1)=pt.y-dila;
        if(pt.z < lowerbound_(2)) lowerbound_(2)=pt.z;
        if(pt.x +dila> upperbound_(0)) upperbound_(0)=pt.x +dila;
        if(pt.y  +dila> upperbound_(1)) upperbound_(1)=pt.y +dila;
...
//     has_map_=true;
// }
// void World::initGridMap_zph(const pcl::PointCloud<pcl::PointXYZ> &cloud)
// {   
//     if(cloud.points.empty())
//     {
//         ROS_ERROR("Can not initialize the map with an empty point cloud!");
