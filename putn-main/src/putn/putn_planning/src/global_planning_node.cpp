...
void rcvPointCloudCallBack1(const sensor_msgs::PointCloud2& pointcloud_map);
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2& pointcloud_map);
void pubInterpolatedPath(const vector<Node*>& solution, ros::Publisher* _path_interpolation_pub);
void findSolution();
void callPlanner();
// void convertPointCloudToWorld(pcl::PointCloud<pcl::PointXYZ>& pointCloud, World& world);
/**
...
void rcvWaypointsCallback(const nav_msgs::Path& wp)
{
  ...
  ROS_INFO("zph");
  ROS_ERROR_STREAM("sdsad"<<wp.poses[0].pose);
  // ROS_INFO(wp.poses[0].pose);
  // ROS_INFO(wp.poses[0].pose.position.y);
}
...
// void convertPointCloudToWorld(pcl::PointCloud<pcl::PointXYZ>& pointCloud) {

//     // 1. 初始化World对象的网格地图

//     world.initGridMap(pointCloud);


//     // 2. 遍历点云，将每个点添加到世界模型中作为障碍物

//     for (const auto& point : pointCloud) {

//         Eigen::Vector3d obstacle(point.x, point.y, point.z);
...
int main(int argc, char** argv)
{
  ...
  ROS_WARN("base_link_id: %s", base_link_id.c_str());
...
////////////////////////////////////////////////zph
 std::string pcdFilePath = "/home/sia/0.pcd"; // 在这里指定PCD文件的路径
  // 读取PCD文件
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);

  if (pcl::io::loadPCDFile<pcl::PointXYZ>(pcdFilePath, *cloud) == -1) { //* 读取文件

      PCL_ERROR("无法读取 %s PCD文件\n", pcdFilePath.c_str());
  ...
  std::cout << "已成功读取720 " << cloud->points.size() << " 个点。" << std::endl;
  // convertPointCloudToWorld(cloud, world);
  ...
  world->initGridMap(*cloud);
    ...
    for (const auto& point : *cloud) {
        ...
        Eigen::Vector3d obstacle(point.x+0.2, point.y, point.z);
    ...
    for (const auto& point : *cloud) {
        ...
        Eigen::Vector3d obstacle(point.x, point.y+0.2, point.z);
...
////////////////////////////////////////////////zph
  ...
  while (ros::ok())
  {
    ...
    while (true && ros::ok())
    {
      try
      {
        listener.lookupTransform("world", base_link_id, ros::Time(0), transform);  //查询变换
        break;
      }
