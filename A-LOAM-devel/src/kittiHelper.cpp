...
void initializeParameters() {
    ...
    nhzph.param<std::string>("camera_init_id", camera_init_id, "camera_init");
    nhzph.param<std::string>("aft_mapped_id", aft_mapped_id, "/aft_mapped");

    // 打印参数以进行验证
    ROS_INFO("camera_init_id: %s", camera_init_id.c_str());
    ROS_INFO("aft_mapped_id: %s", aft_mapped_id.c_str());
}
...
{   
    ...
    odomGT.header.frame_id = camera_init_id;
    odomGT.child_frame_id = "/ground_truth";

    ros::Publisher pubPathGT = n.advertise<nav_msgs::Path> ("/path_gt", 5);
    nav_msgs::Path pathGT;
    pathGT.header.frame_id = camera_init_id;
    ...
    while (std::getline(timestamp_file, line) && ros::ok())
    {
        ...
        laser_cloud_msg.header.frame_id = camera_init_id;
